Public Class Gestion_de_errores
    ...
    'Esta lista representa la fecha en la que han ocurrido los errores
    Public erroresFechas As New List(Of String)

    'Esta lista guardara la informacion sobre cada error
    Public erroresInformacion As New List(Of String)

    'LOAD
    Private Sub Gestion_de_ventas_Load(sender As Object, e As EventArgs) Handles MyBase.Load

        'Indica si se ha repetido alguna fecha a la hora de leer el cotenido del fichero de errores
        ...
        Try
            ...
            'Se carga la lista de errores del fichero de errores
            FileOpen(3, "ErroresSucedidos.txt", OpenMode.Input)

            'Se lee todo el contenido del fichero se guarda en las dos listas
            While Not EOF(3)

                Input(3, errorRegistro.fecha)
                Input(3, errorRegistro.informacionError)
                ...
                For i = 0 To erroresFechas.Count - 1

                    If erroresFechas.Item(i).Equals(errorRegistro.fecha) Then
                ...
                If fechaRepetida = False Then
                    ...
                    ComboBox_dia.Items.Add(errorRegistro.fecha)

                End If

                erroresFechas.Add(errorRegistro.fecha)
                erroresInformacion.Add(errorRegistro.informacionError)
        ...
        Catch ex As System.IO.FileNotFoundException
            ...
            MsgBox("El fichero ""ErroresSucedidos.txt"" no se encuentra por lo tanto no se ha podido leer los errores ocurridos anteriormente" &
           vbCrLf & "Por favor compruebe que el fichero esta en la carpeta de la aplicacion Colchoneria CUESTA. " & vbCrLf &
                    "Ejemplo: Carpeta que contiene la carpeta del programa\Colchoneria-CUESTA\Colchoneria_CUESTA\Colchoneria_CUESTA\bin\Debug\ErroresSucedidos.txt",
                    0 + MsgBoxStyle.Exclamation, "Leer los errores ocurridos durante ejecucion")


        Catch

            MsgBox("Se ha producido un error a la hora de leer la informacion sobre los errores sucedidos guardados en el fichero.",
                   0 + MsgBoxStyle.Information, "Leer los errores ocurridos durante ejecucion")
            ...
            Try
                ...
                errorRegistro.informacionError = Now & " - Se ha producido un error a la hora de leer el fichero ""ErroresSucedidos.txt""" &
        ...
        If erroresFechas.Count = 0 Then

            MsgBox("No ha ocurrido ningun error hasta este momento por lo tanto será dirigido de nuevo a la pantalla de ventas",
                    0 + MsgBoxStyle.Information, "Visualizar errores")
    ...
    Private Sub Button_visualizar_Click(sender As Object, e As EventArgs) Handles Button_visualizar.Click
        ...
        For i = 0 To erroresFechas.Count - 1
            ...
            If erroresFechas.Item(i).Equals(ComboBox_dia.SelectedItem) Then

                TextBox_errores.Text = TextBox_errores.Text & erroresInformacion.Item(i) & vbCrLf & vbCrLf
